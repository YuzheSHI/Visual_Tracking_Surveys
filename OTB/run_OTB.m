function run_OTB(data_path, evalType, trackers)
...
% vector storing fps for every tracker on entire dataset
speed = zeros(1,numTrk);
...
for idxSeq=1:length(seqs)
    ...
    for idxTrk=1:numTrk
        ...
        if isfile([finalPath s.name '_' t.name '.mat'])
        ...
        % store raw results and fps
        if strcmp(evalType, 'OPE')
            raw_result_output(s.name, trackers{idxTrk}.name, res);
            speed(idxTrk) = speed(idxTrk) + res.fps;
        end 
    end
end

% calculate average speed for every tracker on the entire dataset
if strcmp(evalType, 'OPE')
    speed = speed./numSeq;
    speed_output(speed, trackers);
end
