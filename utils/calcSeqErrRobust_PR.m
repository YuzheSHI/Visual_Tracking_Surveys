function [aveErrCoverage, aveErrCoverage_P, aveErrCoverage_R, aveErrCenter, errCoverage, errCoverage_P, errCoverage_R, errCenter] = calcSeqErrRobust_PR(results, rect_anno)
...
[iou, overlap_P, overlap_R] = calcRectInt_PR(rectMat(idx,:),rect_anno(idx,:));

errCoverage=-ones(length(idx),1);
errCoverage(idx) = iou;
errCoverage_P=-ones(length(idx),1);
errCoverage_P(idx) = overlap_P;
errCoverage_R=-ones(length(idx),1);
errCoverage_R(idx) = overlap_R;
errCenter(~idx)=-1;

aveErrCoverage = sum(errCoverage(idx))/length(idx);
aveErrCoverage_P = sum(errCoverage_P(idx))/length(idx);
aveErrCoverage_R = sum(errCoverage_R(idx))/length(idx);
